...
- **MalwareBazaar hash-reputation lookup**: opt-in via `MALWAREBAZAAR_API_KEY`. Computes the file's SHA256 and checks it against abuse.ch's free, community-run database of known-malware hashes - catching samples already confirmed malicious elsewhere that a local ClamAV/YARA/Maldet ruleset hasn't caught up to yet. A confirmed hash match is treated as an authoritative signature hit, exactly like the other three engines (contributes to the same `INFECTED` determination), not a probabilistic/heuristic signal. Results are cached in Redis (`MALWAREBAZAAR_CACHE_TTL_HOURS`, default 24h) so repeat scans of the same file don't cost a fresh API call each time. This check is skipped entirely (no warning, no attempted lookup) if `MALWAREBAZAAR_API_KEY` isn't set.
...
| `SCAN_ENGINE_TIMEOUT_SECONDS` | Maximum time (in seconds) a single YARA, Maldet, or MalwareBazaar invocation may run before being killed and reported as a failed engine - Default `300` (5 minutes). |
| `MALWAREBAZAAR_API_KEY` | Enables the MalwareBazaar hash-reputation check (see above). Get a free key at [bazaar.abuse.ch](https://bazaar.abuse.ch/). Unset (default) disables the check entirely. |
| `MALWAREBAZAAR_CACHE_TTL_HOURS` | How long a MalwareBazaar lookup result (found or not-found) is cached in Redis - Default `24`. Requires `REDIS_URL` to be set; without Redis, every scan does a fresh lookup. |
